...
let connection: aws.codestarconnections.Connection | pulumi.Output<aws.codestarconnections.GetConnectionResult>;
if (config.get("connectionName")) {
  connection = aws.codestarconnections.getConnectionOutput({ name: config.get("connectionName") });
} else {
  connection = new aws.codestarconnections.Connection(identifier, { name: identifier, providerType: "GitHub" });
}
          ...
          configuration: {
            ProjectName: createCodebuild("api-build", "./buildspecs/serverless.yml", {
              environment: {
                ...
                APIENV_APP_DOMAIN: domain || cloudfront.domainName,
